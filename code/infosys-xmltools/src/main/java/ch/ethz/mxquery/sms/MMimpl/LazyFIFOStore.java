...
public abstract class LazyFIFOStore implements MXQueryAppendUpdate {
	...
	protected int granularity = MXQueryGlobals.granularity;
...
//		    	if (si.isEndOfSequence()){
//		    	    endOfSequence = true;
//		    	    return;
//		    	}
		    	
			TokenInterface token = iterator.next();
	...
	public TokenInterface get(int activeTokenId){
	...
	public TokenInterface get(int activeTokenId,int endNode){
		...
		if (currentToken == activeTokenId && currentToken > 0 && current.getFirstTokenId() <= activeTokenId && current.getLastTokenId()>=activeTokenId )  
			if (current.get( activeTokenId, endNode) == Token.END_SEQUENCE_TOKEN)
			    return Token.END_SEQUENCE_TOKEN;		
		
		if (currentToken <= activeTokenId && !endOfStream){
			alreadyThere = false;
			readToken();
		}
...
//		return bufferToItem(nodeId).getTokenIdForNode(nodeId);
	...
	public void buffer(TokenInterface token, int event, boolean isEndOfItem) throws MXQueryException{
		if ( current == null ){
			addNewBufferNode();
		}
	...
	public MXQueryNumber getScoreForItem(int nodeId)
