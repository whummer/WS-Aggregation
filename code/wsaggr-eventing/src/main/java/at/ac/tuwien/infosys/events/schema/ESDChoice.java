...
public class ESDChoice extends ESDObjectWithUnsortedChildren {

	public String toQuery(QueryOutputConfig cfg) {
		int id = cfg.getID();
		System.out.println(getElementName() + " id : " + id);
		String sOuter = getTemplate(getElementName());
		String s = getTemplate("choiceInner");
		String children = "";
		for(Object o : getChildren()) {
			if(o instanceof ESDObject) {
				ESDObject c = (ESDObject)o;
				QueryOutputConfig cfg1 = new QueryOutputConfig(cfg, id);
				String subQuery = c.toQuery(cfg1);
				String inner = s.replace("##self.children##", subQuery);
				children += "(" + inner + "),";
			}
		}
		if(!children.isEmpty()) {
		...
		s = sOuter.replace("##self.children##", children);
